...
wheel 的 `goofish_z/_bundled_skills/` 下；运行时用 `goofish_z.__file__` 拿
...
def _bundle_root() -> Path:
    """wheel 态：site-packages/goofish_z/_bundled_skills；dev 态：repo/skills。"""
    pkg_dir = Path(goofish_z.__file__).resolve().parent
    bundled = pkg_dir / "_bundled_skills"
    if bundled.is_dir():
        return bundled
    # dev 回退：从 src/goofish_z/ 爬到仓库根
